...
class ShapeFrameLayout @JvmOverloads constructor(
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        // 公共
        val radius =
            typedArray.getDimensionPixelOffset(R.styleable.ShapeFrameLayout_fl_radius, 0)
                .toFloat() // 圆角
        val radiusTopLeft = typedArray.getDimensionPixelOffset(
            R.styleable.ShapeFrameLayout_fl_radius_top_left,
            0
        ).toFloat() // 左上圆角
        val radiusTopRight = typedArray.getDimensionPixelOffset(
            R.styleable.ShapeFrameLayout_fl_radius_top_right,
            0
        ).toFloat() // 右上圆角
        val radiusBottomRight = typedArray.getDimensionPixelOffset(
            R.styleable.ShapeFrameLayout_fl_radius_bottom_right,
            0
        ).toFloat() // 右下圆角
        val radiusBottomLeft = typedArray.getDimensionPixelOffset(
            R.styleable.ShapeFrameLayout_fl_radius_bottom_left,
            0
        ).toFloat() // 左下圆角
        val solidColor =
            typedArray.getColor(R.styleable.ShapeFrameLayout_fl_solidColor, 0) // 填充色
        val strokeWidth = typedArray.getDimensionPixelOffset(
            R.styleable.ShapeFrameLayout_fl_strokeWidth,
            0
        ) // 描边宽度
        val strokeColor =
            typedArray.getColor(R.styleable.ShapeFrameLayout_fl_strokeColor, 0) // 描边色
        // 按压
        val pressSolidColor =
            typedArray.getColor(R.styleable.ShapeFrameLayout_fl_pressSolidColor, 0) // 填充色
        // 选中
        val selectedSolidColor =
            typedArray.getColor(R.styleable.ShapeFrameLayout_fl_selectedSolidColor, 0) // 填充色
        val selectedStrokeColor =
            typedArray.getColor(R.styleable.ShapeFrameLayout_fl_selectedStrokeColor, 0) // 描边色
        val startColor = typedArray.getColor(R.styleable.ShapeFrameLayout_fl_startColor, 0)
        val endColor = typedArray.getColor(R.styleable.ShapeFrameLayout_fl_endColor, 0)
